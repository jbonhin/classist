...
$columns = ['id', 'alarm_description', 'equipment_name', 'alarm_class', 'created_at'];
...
if (($resultAlarms) and ($numRows != 0)) {
    while ($row = $resultAlarms->fetch(PDO::FETCH_ASSOC)) {
        ...
        $output['data'] .= '<td>' . $row['alarm_class'] . '</td>';
        $output['data'] .= '<td>' . $row['created_at'] . '</td>';
        $output['data'] .= '<td><a class="btn btn-warning btn-sm" href="formUpAlarm.php?id=' . $row['id'] . '">Editar</a></td>';;
        $output['data'] .= '<td><a class="btn btn-danger btn-sm" href="delAlarm.php?id=' . $row['id'] . '">Excluir</a></td>';
